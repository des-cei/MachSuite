...
int main(int argc, char **argv)
{
  ...
  in_file = "data/input.data";
  #ifdef CHECK_OUTPUT
  check_file = "data/check.data";
  #endif
  if( argc>1 )
    in_file = argv[1];
  #ifdef CHECK_OUTPUT
  ...
  #ifdef WRITE_OUTPUT
  int out_fd;
  out_fd = open("data/output.data", O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
