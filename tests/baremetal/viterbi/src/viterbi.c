...
int viterbi( tok_t obs[N_STATES*N_STATES], prob_t init[N_STATES*N_STATES], prob_t transition[N_STATES*N_STATES], prob_t emission[N_STATES*N_TOKENS], state_t path[N_STATES*N_STATES] )
{
#pragma HLS interface s_axilite port=return
#pragma HLS INTERFACE m_axi depth=64 port=obs offset=slave
#pragma HLS INTERFACE m_axi depth=64 port=init offset=slave
#pragma HLS INTERFACE m_axi depth=4096 port=transition offset=slave
#pragma HLS INTERFACE m_axi depth=4096 port=emission offset=slave
#pragma HLS INTERFACE m_axi depth=64 port=path offset=slave
