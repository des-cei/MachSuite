...
int main(int argc, char **argv)
{
  ...
  in_file = "gemm/ncubed/input.dat"; // in_file = "input.data";
  #ifdef CHECK_OUTPUT
  check_file = "gemm/ncubed/check.dat"; // check_file = "check.data";
  #endif
  if( argc>1 )
    in_file = argv[1];
  #ifdef CHECK_OUTPUT
  ...
  printf("<GEMM/NCUBED> Elapsed time : %.3f ms (average : %.3f ms)\n", t, t/ITERATIONS);

  #ifdef WRITE_OUTPUT
  FIL out_fd; // int out_fd;
  fres = f_open(&out_fd, "gemm/ncubed/output.dat", FA_WRITE | FA_CREATE_ALWAYS); // out_fd = open("output.data", O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
