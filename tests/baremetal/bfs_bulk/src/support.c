...
static inline int fd_printf(FIL fd, const char *format, ...) { // static inline int fd_printf(int fd, const char *format, ...) {
  va_list args;
  int buffered, written, status;
  char buffer[SUFFICIENT_SPRINTF_SPACE];
  FRESULT fres; UINT count;
  ...
  while(written<buffered) {
    fres = f_write(&fd, &buffer[written], buffered-written, &count); // status = write(fd, &buffer[written], buffered-written);
    assert(count>=0 && "Write failed"); // assert(status>=0 && "Write failed");
    written += count; // written += status;
  }
...
char *readfile(FIL fd) { // char *readfile(int fd) {
  ...
  len = fd.fsize; // len = s.st_size;
  assert(len>0 && "File is empty");
  p = (char *)malloc(len+1);
  bytes_read = 0;
  while( bytes_read<len ) {
    fres = f_read(&fd, &p[bytes_read], len-bytes_read, &count); // status = read(fd, &p[bytes_read], len-bytes_read);
    assert(count>=0 && "read() failed"); // assert(status>=0 && "read() failed");
    bytes_read+=count; // bytes_read+=status;
  }
...
int write_string(FIL fd, char *arr, int n) { // int write_string(int fd, char *arr, int n) {
  ...
  written = 0;
  while(written<n) {
    fres = f_write(&fd, &arr[written], n-written, &count); // status = write(fd, &arr[written], n-written);
    assert(count>=0 && "Write failed"); // assert(status>=0 && "Write failed");
    written += count; // written += status;
  }
  // Write terminating '\n'
  do {
    fres = f_write(&fd, "\n", 1, &count); // status = write(fd, "\n", 1);
    assert(count>=0 && "Write failed"); // assert(status>=0 && "Write failed");
  } while(count==0); // while(status==0);
...
//}
#define generate_write_TYPE_array(TYPE, FORMAT) \
int write_##TYPE##_array(FIL fd, TYPE *arr, int n) { \
  int i; \
  for( i=0; i<n; i++ ) { \
    fd_printf(fd, "%" FORMAT "\n", arr[i]); \
  } \
