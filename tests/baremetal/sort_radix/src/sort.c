...
void ss_sort(int a[SIZE], int b[SIZE], int bucket[BUCKETSIZE], int sum[SIZE]){
...
#pragma HLS INTERFACE m_axi depth=2048 port=bucket offset=slave
#pragma HLS INTERFACE m_axi depth=128 port=sum offset=slave
    int exp=0;
    int valid_buffer=0;
    #define BUFFER_A 0
    #define BUFFER_B 1
