...
void aes256_encrypt_ecb(uint8_t key[32], uint8_t enckey[32], uint8_t deckey[32], uint8_t k[32], uint8_t buf[32]) {
#pragma HLS interface s_axilite port=return
#pragma HLS INTERFACE m_axi depth=32 port=key offset=slave
#pragma HLS INTERFACE m_axi depth=32 port=enckey offset=slave
#pragma HLS INTERFACE m_axi depth=32 port=deckey offset=slave
#pragma HLS INTERFACE m_axi depth=32 port=k offset=slave
#pragma HLS INTERFACE m_axi depth=32 port=buf offset=slave
    //INIT
    uint8_t rcon = 1;
    uint8_t i;
